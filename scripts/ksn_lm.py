...
class KneserNeyLM:
	...
	def train(self, ngrams):
		...
		probs = self.calc_probs(kgram_counts)
		return probs
	...
	def score_sent(self, sent,base_2=True):
		"""
		Return log prob of the sentence.
		Params:
			sent [tuple->string] The words in the unpadded sentence.
		...
		for i in range(len(padded) - self.highest_order + 1):
			...
			prob = self.logprob(ngram)
			if base_2:
				prob*=math.log(math.e,2)
			sent_logprob +=prob
		return sent_logprob
	...
	'''def generate_sentence(self, min_length=4):
		...
		return sent'''
	def unconditioned_text_generate(self, length=10):
		"""
		Generate a sentence using the probabilities in the language model.

		Params:
			length [int] The number of words in the generated sentence.
		"""
		sent = []
		probs = self.highest_order_probs()
		sent = [self.start_pad_symbol] * (self.highest_order - 1)
		for i in range(length):
			# Append first to avoid case where start & end symbal are same
			sent.append(self.generate_next_word(sent, probs))
		sent = ' '.join(sent[(self.highest_order - 1):])
		#return sent[(self.highest_order - 1):]
		return sent

	def conditioned_text_generate(self,context,length=10):
		sent=list(context)
		probs = self.highest_order_probs()
		for i in range(length):
			sent.append(self.generate_next_word(context,probs))
		sent = ' '.join(sent)
		return sent
